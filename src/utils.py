...
def update_table(table_name, df, retries=28, delay=500):
    """
    ...
    if 'insertion_date' in df_clean.columns:
        # Convert to datetime object first, then format it as a string in SQL Server format
    ...
    while attempt < retries:
        try:
            with engine.connect() as connection:
                df_clean.to_sql(f'{table_name}', engine, index=False, if_exists='append', chunksize=1000, schema='dbo')
            ...
            if attempt < retries:
                ...
                logger.error("All retries failed. Please check the error and try again later.")
